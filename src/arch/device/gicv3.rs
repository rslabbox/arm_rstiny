...
use crate::{
    arch::mem::phys_to_virt,
    config::devices::{GICD_BASE, GICR_BASE},
};
...
pub fn irq_init() {
    ...
    let gicd_base_address: *mut Gicd = phys_to_virt(pa!(GICD_BASE)).as_mut_ptr_of();
    let gicr_base_address: *mut GicrSgi = phys_to_virt(pa!(GICR_BASE)).as_mut_ptr_of();
