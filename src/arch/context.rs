...
impl TrapFrame {
    ...
    pub unsafe fn exec(&self, kstack_top: VirtAddr) -> ! {
        ...
        mov     sp, {kstack}
        ldp     x30, x9, [{ctx}, 30 * 8]
        ldp     x10, x11, [{ctx}, 32 * 8]
        msr     sp_el0, x9
        msr     elr_el1, x10
        msr     spsr_el1, x11

        ldp     x28, x29, [{ctx}, 28 * 8]
        ldp     x26, x27, [{ctx}, 26 * 8]
        ldp     x24, x25, [{ctx}, 24 * 8]
        ldp     x22, x23, [{ctx}, 22 * 8]
        ldp     x20, x21, [{ctx}, 20 * 8]
        ldp     x18, x19, [{ctx}, 18 * 8]
        ldp     x16, x17, [{ctx}, 16 * 8]
        ldp     x14, x15, [{ctx}, 14 * 8]
        ldp     x12, x13, [{ctx}, 12 * 8]
        ldp     x10, x11, [{ctx}, 10 * 8]
        ldp     x8, x9, [{ctx}, 8 * 8]
        ldp     x6, x7, [{ctx}, 6 * 8]
        ldp     x4, x5, [{ctx}, 4 * 8]
        ldp     x2, x3, [{ctx}, 2 * 8]
        ...
        eret",
            ctx = in(reg) self,
            kstack = in(reg) kstack_top.as_usize(),
            options(noreturn),
        )
    }
}
