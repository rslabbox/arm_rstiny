...
use crate::drivers::pci::realtek::{EthernetAddress, NetBufPtr};
...
impl Rtl8139Driver {
    ...
    pub fn new(base_addr: usize, irq: u8) -> TinyResult<Self> {
        Ok(Self {
    ...
    /// Receive a packet
    fn do_receive(&mut self) -> TinyResult<super::NetBufPtr> {
        // Check if packet available
        let cmd = self.mmio.read8(CR);
        if (cmd & CR_BUFE) != 0 {
            return Err(TinyError::Again); // Buffer empty
        }
        ...
        Ok(super::NetBufPtr::new(raw_ptr, raw_ptr, packet_len))
    }
...
impl super::BaseDriverOps for Rtl8139Driver {
    fn device_name(&self) -> &str {
        "rtl8139"
    }

    fn device_type(&self) -> super::DeviceType {
        super::DeviceType::Net
    }
}

impl super::NetDriverOps for Rtl8139Driver {
