...
use crate::config::kernel::{KIMAGE_VADDR, kernel_phys_base};
...
pub fn phys_to_virt(paddr: PhysAddr) -> VirtAddr {
    ...
    if kbase > 0 && pa >= kbase {
        ...
        va!(pa.wrapping_sub(kbase).wrapping_add(KIMAGE_VADDR))
    } else {
...
pub fn virt_to_phys(vaddr: VirtAddr) -> PhysAddr {
    ...
    if va >= KIMAGE_VADDR {
        ...
        pa!(va.wrapping_sub(KIMAGE_VADDR).wrapping_add(kernel_phys_base()))
    } else {
