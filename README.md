...
Cargo workspace 包含 `bootloader`、`kernel`、`projects/apps/fatboot`、`projects/apps/hello`、`projects/libs/abi`、用户 API `projects/libs/user` 和运行库 `projects/libs/runtime`，引导程序、内核与用户程序分别使用链接脚本。内核可独立 Cargo 构建，不再嵌入用户程序；直接 Cargo 构建生成固定的 EL1 平台配置到 OUT_DIR，因而也需要 QEMU 和 dtc 工具。通过 `make build` 生成完整启动镜像；QEMU 必须启动 bootloader，`kernel.bin` 仅保留为辅助产物。直接构建 bootloader 时必须提供 `BOOT_ARCHIVE` 才能得到可启动镜像，详见 [Rust 引导链](docs/boot.md)。
