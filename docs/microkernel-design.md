...
第一版构建 `user/fatboot` 为独立 ELF；宿主打包工具验证其装载段，生成带入口、段描述和字节内容的 boot module，与内核打包。内核只映射/拷贝描述中的段并建立首个任务，不读取 FAT32，也不提供通用磁盘 ELF loader。
...
- 独立 TTBR0 用户页表；TTBR1 保留仅 EL1 可访问的内核映射。
...
- [ ] 为后续用户 ELF 增加独立 linker/CRT/ABI crate。
