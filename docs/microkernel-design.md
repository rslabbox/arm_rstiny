...
日期：2026-09-08。状态：构建基线、内核底座和最小 EL0 fatboot 已实施，能力系统及后续部分为设计提案。参见 [内核实现与验证记录](kernel-implementation.md) 和 [fatboot 启动文档](fatboot.md)。
...
当前实现进度：内核底座与最小 EL0 fatboot 已完成。实际 ABI、低地址内核映射的调整和验证见 [fatboot 启动文档](fatboot.md)。下文旧代码分析保留为设计背景，后续章节为路线规划。
...
2. 固定 AArch64、单核、QEMU `virt`；显式选择初期 GIC 版本，例如 GICv2，并确保对应实现一致。
...
- 独立 TTBR0 用户页表；长期规划用 TTBR1 放置仅 EL1 可访问的内核映射。当前实现保留 TTBR0 中 supervisor-only 的低地址内核映射，TTBR1 禁用；用户与内核访问权限已分别验证。
- 用户 text RX、data/BSS RW/NX、用户栈 RW/NX，栈旁留 guard page。
- 用户只读 BootInfo 和专用 IPC buffer；物理页不与内核对象重叠。
- 初始寄存器：ELR_EL1 = 入口、SP_EL0 = 对齐后的用户栈顶、SPSR_EL1 = 合法 EL0t 状态、x0 = BootInfo 用户虚拟地址；其他寄存器清零。
