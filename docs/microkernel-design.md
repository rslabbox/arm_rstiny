...
因此，参考系统目前是“fatboot 内含块设备/FAT32 用户态库 + 独立串口服务”，并非每个驱动都已经独立成进程。先复现这一粒度，再考虑拆成 `block-server` 和 `fs-server`。
...
| 设备中断与服务拆分（已实现 2026-09-10） | IRQ 通知，块设备/文件系统可独立服务 | 完整 fatboot | SMP、网络 |
...
## 14. 设备中断与服务拆分：设备 IRQ 与进一步拆服务（已实现 2026-09-10）

实现设计（对象、ABI、GIC 时序、block-server 迁移、验收）见 [设备 IRQ 授权与用户态投递](irq.md)，实施记录见其 §13。

多线程与抢占 已实现调度 timer IRQ；用户设备 IRQ 授权与交付已实现：`IRQControl`/`IRQHandler` 对象、split EOI 投递、userboot→init→驱动授权链、block-server 通知化读盘。
...
然后逐步拆分：`fatboot → block-server → fs-server → 应用`。块数据和文件数据经授权共享 Frame 搬运，IPC 只传请求描述、长度、偏移和完成状态；定义缓冲区所有权、最大并发、超时和服务重启后的失效规则。
