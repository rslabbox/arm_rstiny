...
当前实现进度：内核底座与最小 EL0 fatboot 已完成，现已补齐 [单核用户内存与任务调度](memory-task.md) 和 [原版 seL4 elfloader 引导链](boot.md)。当前父子任务句柄不是完整 CSpace；原路线中的能力系统仍待实现。实际 ABI、高地址内核映射和验证见 [fatboot 启动文档](fatboot.md)。下文旧代码分析保留为设计背景，后续章节为路线规划。
...
当前已采用原版 seL4 ARM elfloader：下述 EL 转换、临时映射和启用 MMU 由 loader 完成，内核以 MMU/cache 已开启的 EL1 状态接收六个启动参数。运行期页表和 root task 初始化由 Rust 内核负责，详见 [引导链](boot.md)。
