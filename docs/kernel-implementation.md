...
| 故障 | 每个向量入口记录 kind/source；ESR/FAR/寄存器先保存后打印；正常完成、异常与 panic 最终调用 PSCI 关机；固件返回时停驻兜底 |
...
- dev/release × LOG=info/off，共四种正式构建，全部到达 `kernel_shutdown`，继续执行后 QEMU 实际以状态 0 退出。
