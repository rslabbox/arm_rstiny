...
extern "C" fn fatal_exception(frame: &TrapFrame, kind: u64, source: u64) -> ! {
    ...
    // source 0/1 = current EL; 2/3 = lower EL. User faults terminate only their task.
    ...
    if source == 2 {
        crate::task::dispatch(frame, crate::task::Event::Fault(esr));
    }
...
extern "C" fn handle_user_sync(frame: &mut TrapFrame) -> ! {
    ...
    if esr >> 26 == 0x15 && esr & 0xffff == 0 {
        crate::task::dispatch(frame, crate::task::Event::Syscall);
...
#[unsafe(no_mangle)]
extern "C" fn handle_irq(frame: &TrapFrame) -> ! {
    super::irq::handle();
    crate::task::dispatch(frame, crate::task::Event::Timer)
}
