...
from check_userboot import run as run_chain
...
def main():
    ...
    for mode in ('debug', 'release'):
        ...
        for minimum in (0x41000000, 0x43e00000):
            ...
            # The supervised boot chain must survive kernel relocation.
            run_chain(args.qemu, kernel)
        ...
        for root_base in (0x800000, 0x3000000):
            try:
                ...
                user = ROOT / f'target/apps/{mode}/{TARGET}/{mode}/userboot'
        ...
        build(mode, 'info', False, 0)
    print('PASS: identical kernel ELF supports physical relocation and different root VAs; mappings, supervised chain, exhaustion.', flush=True)
