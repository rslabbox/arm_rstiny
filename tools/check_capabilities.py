    ...
    with tempfile.TemporaryDirectory(prefix='rstiny-caps-') as temporary:
        ...
        try:
            ...
            def retype(kind,slot,count=1,bits=0,status=0):
                return c.call(16,1,[kind,bits,0,0,slot,count],[2],status)
            ...
            retype(7,40)
            retype(7,40,status=8)
            retype(2,41,status=1) # Endpoint objects are explicitly unsupported
            c.call(16,1,[7,0,0,0,41],[2],status=7)
            c.call(40,11,status=3) # a frame is not a TCB
            c.call(2,18,[40,32],status=6)

            scratch = 0x07000000
            retype(9,41)
            c.call(40,40,[scratch,3,5],[3],status=6) # no L3 table
            c.call(41,38,[scratch,1],[3])
            c.copy(2,49,41)
            c.call(49,38,[scratch+0x200000,1],[3],status=1)
            delete(49)
            c.call(40,40,[scratch+1,3,5],[3],status=5)
            c.call(40,40,[scratch,3,5],[3])
            physical = c.call(40,46)
            c.copy(2,42,40,rights=2)
            c.call(42,40,[scratch+4096,3,5],[3])
            mapping = pages(g)
            ...
            c.call(40,40,[scratch+8192,3,5],[3],status=1) # one mapping per frame cap
            c.copy(2,43,42,rights=15)
            c.call(43,40,[scratch+8192,3,5],[3])
            assert pages(g)[scratch+8192][1] & (1 << 7), 'descendant regained write'
            revoke(42)
            c.call(43,46,status=6)
            assert scratch+8192 not in pages(g)
            c.call(40,41)
            delete(40) # original cap deletion does not revoke independent descendants
            assert c.call(42,46) == physical
            c.call(42,41); delete(42); delete(41)
            ...
            c.copy(2,50,16)
            c.call(50,1,[7,0,0,0,51,1],[2])
            c.call(50,1,[9,0,0,0,52,1],[2])
            retype(7,53); retype(7,54)
            c.call(52,38,[scratch,1],[3])
            c.call(51,40,[scratch,3,5],[3])
            c.call(53,40,[scratch+4096,3,5],[3])
            c.call(2,17,[50,64],status=3) # unrelated frame keeps L3 nonempty
            assert scratch not in pages(g)
            c.call(54,40,[scratch,3,5],[3])
            delete(51) # already unmapped: must leave cap 54's mapping intact
            assert pages(g)[scratch][0] == c.call(54,46)
            c.call(53,41); c.call(54,41)
            revoke(50); delete(50); delete(53); delete(54)
            assert c.runtime('available') == before
            ...
            retype(7,60,2); retype(9,62)
            c.call(62,38,[scratch,1],[3])
            c.call(60,40,[scratch,3,5],[3])
            c.runtime('unmap',1,scratch,4096)
            c.call(61,40,[scratch,3,5],[3])
            c.call(60,40,[scratch,2,5],[3],status=2)
            c.call(60,41)
            assert pages(g)[scratch][0] == c.call(61,46)
            delete(60); c.call(61,41); delete(61); delete(62)
            assert c.runtime('available') == before
